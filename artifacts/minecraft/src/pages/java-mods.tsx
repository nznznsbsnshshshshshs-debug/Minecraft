...
export default function JavaMods() {
  const { data: mods, isLoading } = useListMods({ category: "java" });
      ...
      <div className="min-h-screen bg-black text-white pb-24">
        <div className="relative overflow-hidden px-5 pt-14 pb-8">
          ...
          <div className="absolute inset-0 pointer-events-none"
            style={{ backgroundImage: "linear-gradient(rgba(59,130,246,0.025) 1px, transparent 1px), linear-gradient(90deg, rgba(59,130,246,0.025) 1px, transparent 1px)", backgroundSize: "32px 32px" }} />
          <div className="relative z-10 max-w-2xl mx-auto">
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}>
              <Link href="/mods">
                <span className="flex items-center gap-1 text-zinc-500 hover:text-blue-300 text-sm mb-5 w-fit transition-colors cursor-pointer">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
              ...
              <div>
                <h1 className="text-2xl font-black text-white">Java Mods</h1>
                <p className="text-zinc-400 text-sm">Shaders, textures & gameplay upgrades</p>
              </div>
            </motion.div>
          </div>
        </div>
